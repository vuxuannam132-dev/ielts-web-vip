...
import { Upload, Plus, Trash2, Save, Loader2, CheckCircle2, ArrowLeft, GraduationCap, FileJson, X, Sparkles } from "lucide-react";
        ...
        <div className="min-h-[calc(100vh-4rem)] bg-gradient-to-br from-slate-50 to-slate-100">
            <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-6">
                ...
                <div className="flex items-center gap-3 bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                    ...
                    <button onClick={() => setShowJsonModal(true)} className="flex items-center gap-2 bg-violet-100 hover:bg-violet-200 text-violet-700 px-4 py-2 rounded-lg text-sm font-semibold transition">
                    ...
                    <div className="flex gap-2 flex-wrap">
                        {[
                            ...
                            <button key={s.key} onClick={() => {
                                setSkill(s.key);
                                if (parsedSkillsMap[s.key]) {
                                    loadSkillFromData(s.key, parsedSkillsMap[s.key]);
                                }
                            }}
                ...
                <div className="fixed inset-0 bg-slate-900/50 flex items-center justify-center p-4 z-50">
                    <div className="bg-white rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl">
                        <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50">
                            <h3 className="font-bold text-slate-800 flex items-center gap-2"><Sparkles className="h-5 w-5 text-violet-600"/> Smart Import</h3>
                                ...
                                <div className="absolute inset-0 z-10 bg-white/95 backdrop-blur-md flex flex-col items-center justify-center p-6 rounded-b-2xl animate-fade-in">
                                    <div className="relative w-24 h-24 flex items-center justify-center mb-3">
                                        ...
                                        <div className="absolute flex flex-col items-center justify-center">
                                            <span className="text-xl font-extrabold text-violet-700 font-mono tracking-tight">{parseProgress}%</span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-2.5 bg-violet-50 border border-violet-100 px-3.5 py-1.5 rounded-full text-xs font-medium text-violet-800">
                        ...
                        <div className="p-4 border-t border-slate-100 flex justify-end gap-3 bg-slate-50">
                            ...
                            <button onClick={handleAIImport} disabled={!jsonImportText.trim() || isParsingAI} className="px-5 py-2 bg-violet-600 hover:bg-violet-700 disabled:opacity-50 text-white font-semibold rounded-xl transition text-sm flex items-center gap-2">
